...
calls per generated word, with longer replies and no empty ones. The claim is an existence proof, not
a competitor to language models: the replies are short, simple and slow.
\end{abstract}
...
across the system's development (Section~\ref{sec:results}), and an honest account of the limits
...
file behind the numbers here will be released at \url{https://github.com/xucian/talktojev}.
...
them. Nothing in the loop is a template or a phrase bank, and no rule chooses a word: when the
...
number, the end of the sentence, or one of \Categories{} kinds of content word? If a kind: which
word of that kind? \\
...
day before. The self-critique stayed flat, which we read as a ceiling of the measure rather than of
the system: short replies score 6 almost always, and a longer reply has more room for one slip.
...
control shows the score is not empty (it gives \CtrlCrit{} to word salad), but it cannot show that
the score tracks what a person would prefer. We have no human evaluation yet. Most comparisons in Section~\ref{sec:principles} rest on four to eight prompts run
